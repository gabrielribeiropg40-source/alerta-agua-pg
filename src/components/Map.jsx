        ...
        {reports.map(report => {
          ...
          return (
            <Marker 
              ...
              icon={report.tipo === 'falta_agua' ? shortageIcon : (isAlert ? alertIcon : new L.Icon.Default())}
            >
              <Popup>
                <div style={{ textAlign: 'center' }}>
                  <strong>{report.bairro}</strong><br/>
                  {report.tipo === 'falta_agua' ? (
                    <span style={{ color: 'var(--danger-color)', fontWeight: 'bold' }}>Falta de Água</span>
                  ) : (
                    <span>Cor: {report.cor_agua}</span>
                  )}<br/>
